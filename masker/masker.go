...
import (
	...
	"github.com/3mre0s/Anonmyz/config"
	"github.com/3mre0s/Anonmyz/metrics"
	"github.com/3mre0s/Anonmyz/patterns"
	"github.com/3mre0s/Anonmyz/vault"
)
...
// generateLabel returns a random, bracket-delimited placeholder candidate.
// Store rejects collisions and storeWithUniqueLabel retries before use.
...
// storeWithUniqueLabel reserves a fresh placeholder without ever overwriting an
// existing vault entry.
func (m *Masker) storeWithUniqueLabel(prefix, original string) (string, error) {
	const maxAttempts = 16
	for attempt := 0; attempt < maxAttempts; attempt++ {
		label := generateLabel(prefix)
		err := m.vault.Store(label, original)
		if err == nil {
			return label, nil
		}
	...
	return "", fmt.Errorf("could not allocate a unique placeholder after %d attempts", maxAttempts)
}
...
func (m *Masker) maskFull(text string, p patterns.SensitivePattern, r *MaskResult, seen map[string]string) string {
	return p.Regex.ReplaceAllStringFunc(text, func(match string) string {
		...
		label, err := m.storeWithUniqueLabel(p.Prefix, match)
		if err != nil {
...
func (m *Masker) maskGroup(text string, p patterns.SensitivePattern, r *MaskResult, seen map[string]string) string {
	return p.Regex.ReplaceAllStringFunc(text, func(match string) string {
		...
		label, err := m.storeWithUniqueLabel(p.Prefix, value)
		if err != nil {
			log.Printf("[WARN] vault full (kasa dolu) — %s left unmasked: %v", p.Name, err)
			metrics.Global.IncVaultEvictions()
			r.VaultEvictions++
