...
func TestVaultRejectsLabelCollisionWithoutOverwriting(t *testing.T) {
	t.Parallel()

	v := New(2)
	label := "[[SECRET_DEADC0DE]]"
	if err := v.Store(label, "first-secret"); err != nil {
		t.Fatalf("first store failed: %v", err)
	}
	if err := v.Store(label, "second-secret"); !errors.Is(err, ErrLabelExists) {
		t.Fatalf("duplicate store error = %v, want ErrLabelExists", err)
	}

	got, found := v.Retrieve(label)
	if !found || got != "first-secret" {
		t.Fatalf("collision overwrote vault entry: got %q (found=%v)", got, found)
	}
}
